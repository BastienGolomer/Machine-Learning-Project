...
def K_fold( y, X_train, K = 10):
    ...
    w_K_fold_i=[]
    losses=[]
    size_w = valx[0].shape[1]

    w_temp = np.ones(size_w)/10
    ...
    for i in range (0,K):
        ...
        [w_temp, loss] = logistic_regression(trainy[i],trainx[i],w_temp,100,0.1)
        ...
        # [w_temp, loss] = reg_logistic_regression(trainy[i],trainx[i],1.1,np.random.rand(size_w),100,0.1)
        ...
        print (compute_confusion_matrix(valy[i], valx[i].dot(np.array(w_temp).T)))
